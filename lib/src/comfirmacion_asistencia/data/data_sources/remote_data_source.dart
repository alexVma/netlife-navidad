...
class RemoteDataSource implements ConfirmationDS {
  ...
  User getUser(String userId) {
    Map<String, User> mockUsers = {
      'test1': User(
          id: '', nombre: 'Alex', apellido: 'Minga', celular: '0992628036'),
      'test2': User(
          id: '',
          nombre: 'Nombre',
          apellido: 'Apellido',
  ...
  Future<User> getUserByIdentification(String identification) async {
    final url =
        '${AppConfig.BACKEND_URL}usuario/findByCedula/{cedula}?cedula=$identification';
    User user = User(
        id: '0',
        apellido: 'vuelva a intentar',
        nombre: 'Identificacion No existe',
        celular: '',
        ciudadEvento: '');
    try {
      ...
      if (response.statusCode == 200) {
        if (response.body.length > 1) {
          ...
          user = User(
              id: jsonResponse['id'],
              apellido: jsonResponse['apellidos'],
